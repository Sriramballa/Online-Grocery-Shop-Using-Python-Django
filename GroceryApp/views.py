...
def profile(request):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            pass
        user = User.objects.filter(id=request.user.id).update(first_name=fname, last_name=lname, username=email)
...
def user_feedback(request):
    ...
    return render(request, "feedback_form.html", locals())
...
def delete_order(request, pid):
    ...
    return redirect('/manage_order/?action='+request.GET.get('action'))
...
def admin_order_track(request, pid):
    ...
    return render(request, 'admin_order_track.html', locals())
